...
class Tree:
    ...
    def set_outputs(self,
        ...
        if node is None:
            ...
            node.left = self.set_outputs(node.left, left_output_classes, current_depth)
            ...
            node.right = self.set_outputs(node.right, right_output_classes, current_depth)
    ...
    def predict(self,
        ...
        if one_hot:
            oh = np.zeros((results.size, len(self.targets)))
            oh[np.arange(results.size), results] = 1
    ...
    def show(self):
        ...
        pos = None  # graphviz_layout(graph, prog="dot")
